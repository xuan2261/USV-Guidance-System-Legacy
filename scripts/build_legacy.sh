...
# Ubuntu/Debian GeographicLib development packages ship FindGeographicLib.cmake
# outside CMake's default module search path. Discover it from dpkg instead of
# patching the pinned upstream source tree, then pass its directory to every
# catkin package through the workspace CMake configuration.
GEO_FIND="$(dpkg -L libgeographic-dev libgeographiclib-dev 2>/dev/null | grep '/FindGeographicLib\.cmake$' | head -n1 || true)"
GEO_MODULE_DIR=""
if [[ -n "$GEO_FIND" ]]; then
  GEO_MODULE_DIR="$(dirname "$GEO_FIND")"
fi
python3 - "$RET/geographiclib_discovery.json" "$GEO_FIND" "$GEO_MODULE_DIR" <<'PY'
import json,sys
find_path,module_dir=sys.argv[2:]
status='PASS' if find_path and module_dir else 'FAIL'
with open(sys.argv[1],'w') as f:
    json.dump({'status':status,'find_module':find_path or None,'cmake_module_path':module_dir or None},f,indent=2); f.write('\n')
PY
...
status='PASS' if br==0 and cr==0 else 'FAIL'
with open(out,'w') as f:
    json.dump({'status':status,'rosdep_rc':rr,'catkin_config_rc':cr,'catkin_build_rc':br,'geographiclib_module_path':geo or None},f,indent=2); f.write('\n')
PY
exit "$BUILD_RC"
