...
int main()
{
  ...
  require(std::abs(point_to_aabb_distance_m(Point2dM{13.0, 14.0}, box) - 5.0) < 1e-12, "AABB metric distance failed");
  require(std::abs(point_to_segment_distance_m(Point2dM{5.0, 3.0}, Segment2dM{{0.0, 0.0}, {10.0, 0.0}}) - 3.0) < 1e-12, "segment metric distance failed");
  ...
  for (int i = 0; i < 1000; ++i) {
    require(tree.query_point(Point2dM{50.0, 50.0}) == std::vector<Quadtree::ItemId>({30}), "quadtree split-boundary regression failed");
  }
  ...
  require(map.distance_m(Point2dM{100.0, 100.0}, LayerId::Collision, 20.0) == 20.0, "distance saturation failed");
  ...
  require(std::abs(map.voronoi_field(Point2dM{20.0, 22.0}) - expected) < 1e-12, "legacy Voronoi formula regression failed");
